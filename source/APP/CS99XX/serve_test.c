...
l:
	...
	{
		GUI_DispStringAt((const char*)"日期:", 0+X0+35, Y0+4);
		GUI_DispStringAt((const char*)data_buf, 0+X0+35+6*5, Y0+4);
		GUI_DispStringAt((const char*)"时间:", 0+X0+35, Y0+4+14);
...
static void set_offset_mode(void)
{
	...
	uint16_t* pkey_value = NULL;
	INT8U err = 0;
	uint8_t n = 1;//g_offset_mode;//默认单次模式
	...
	if(sys_par.language == CHINESE)
	{
		GUI_DispStringAt((const char*)"偏移测试模式", X0+30, Y0+3);
	}
	else
	{
	...
	if(sys_par.language == CHINESE)
	{
		GUI_DispStringAt((const char*)"单次模式", X0+15, Y0+30);
		GUI_DispStringAt((const char*)"连续模式", X0+15+60, Y0+30);
	}
	else
	{
		GUI_SetFont(&GUI_FontHZ_SimSun_13);
	...
	while(1)
	{
		switch(n)
		{
			case OFFSET_SIG:/* 单步模式 */
				GUI_DrawBitmap((const GUI_BITMAP*) &bmcircle_,X0+5, Y0+30+3);
				GUI_DrawBitmap((const GUI_BITMAP*) &bmcircle, X0+5+60, Y0+30+3);
				cursor_pos(X0+5+1, Y0+30);
				break;
			case OFFSET_CON:/* 连续模式 */
...
/*
 * 函数名：test_unit_dis
 * 描述  ：测试界面单位显示，由于单位显示使用的字体与数据字体不一样因此要单独显示
 * 输入  ：mode 测试模式
 *         gear 档位
 *         pNewFont  字体
 * 输出  ：无
...
static void serve_test_more(void)
{
	...
	if(sys_par.language == CHINESE)
	{
		GUI_DispStringAt((const char*)"偏移", 208, 2);
		GUI_DispStringAt((const char*)"容值", 208, 18);
		GUI_DispStringAt((const char*)"结果", 208, 34);
	}
	else
	{
	...
	while(1)
	{
		...
		switch(*pkey_value)
		{
			...
			case KEY_F3: /* 结果显示 */
				sys_result(0);
				g_exit = 1;	/* 为了标示为是子菜单中按下了返回键而不是本级菜单 */
				break;
			case KEY_BACK:
				g_return = 1;
				return;
...
l:	GUI_ClearRect (200+1, 0+0, 240-1, 15-1);
	...
	if(sys_par.language == CHINESE)
	{
		GUI_DispStringAt((const char*)"内存", 208, 2);
		GUI_DispStringAt((const char*)"设置", 208, 18);
	...
	while(1)
	{
		...
		{
			case KEY_TEST_MEM:
				sys_memory(1);/* 进入内存管理界面 1表示从测试界面进入的 */
				Lcd24064_Clr();
				break;
			case KEY_TEST_SET:
				set_subtask(1);/* 进入设置界面 1表示是从测试界面调用的 */
				Lcd24064_Clr();
				break;
			case KEY_TEST_MORE:
				serve_test_more();
				if(g_return && g_exit)
				{/* 当g_return 和g_exit同时为1时表示在最低层菜单完成操作后返回，应该继续向上返回 之前 需将g_exit清零 */
					/* 也可以用判断返回值的方法来判断 */
					Lcd24064_Clr();
					g_exit = 0;
					break;
		...
		if(g_exit || g_return)
		{
			...
			{/* 如果当前步被改变了那么在返回时应恢复 */
				...
				p = position_step(backup);/* 必须要对p的值进行判断 否则后果很严重 */
				if(p != NULL)
				{
					g_cur_step = p;
				}
...
// 				test_reset();
				...
				exit_test_relay_motion();/* 继电器进入就绪状态 */
