...
#define VERSION_NUMBER__    "2.20.74"	/* 版本号 */
...
void prompt_recover_type(const int8_t s)
{
	...
	if(s == -1)
	{
		GUI_DispStringAt("系统型号校验错", 0+X0+35, Y0+4);
		GUI_DispStringAt("误！请检查硬件", 0+X0+35, Y0+4+14);
		GUI_DispStringAt("配置后重试。", 0+X0+35, Y0+4+14+14);
	}
	else
	{
		GUI_DispStringAt("数据删除中，按", 0+X0+35, Y0+4);
		GUI_DispStringAt("键无效，不要关", 0+X0+35, Y0+4+14);
		GUI_DispStringAt("机！请稍候...", 0+X0+35, Y0+4+14+14);
	}
...
void select_type(void)
{
	...
	int32_t size = sizeof(type_pool)/sizeof(type_pool[0]) - 1;//去除最后一个空元素
