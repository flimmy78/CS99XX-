...
void gui_draw_dc_gr_module_comm_fail(void)
{
	...
	if(sys_par.language == CHINESE)
	{
		GUI_DispStringAt((const char*)" 直流模块通信异常 ", X0+35, Y0+10);
		GUI_DispStringAt((const char*)"", X0+35, Y0+16);
		GUI_DispStringAt((const char*)" 请按Enter键继续", X0+35, Y0+28);
	}
	else
	{
		GUI_DispStringAt((const char*)"DC_GR_M_Comm.Fail.", X0+35, Y0+4);
		GUI_DispStringAt((const char*)"Press Enter key ", X0+35, Y0+16);
		GUI_DispStringAt((const char*)"to continue.", X0+35, Y0+28);
	}
    
	GUI_DrawBitmap((const GUI_BITMAP*) &bmwarning, X0 + 5, Y0 + 10);
	ReFresh_LCD24064();
    
	while(scan_keyboard() != KEY_ENTER);
}
...
void gui_draw_dc_gr_module_being_in_comm(void)
{
	...
	if(sys_par.language == CHINESE)
	{
		GUI_DispStringAt((const char*)"模块正在通信 ", X0+35, Y0+4);
		GUI_DispStringAt((const char*)"需要等候1分钟时间...", X0+35, Y0+17);
		GUI_DispStringAt((const char*)"按EXIT取消!", X0+35, Y0+30);
	}
	else
	{
		GUI_DispStringAt((const char*)"Module Being In", X0+35, Y0+4);
		GUI_DispStringAt((const char*)"Communication.", X0+35, Y0+17);
		GUI_DispStringAt((const char*)"Please Wait 1 minutes.", X0+35, Y0+30);
	}
...
#ifdef RH8400AS_DZ //深圳西思特科技定制机
void xi_si_te_ke_ji_main_ui(void)
{
	char t_buf[50] = {0};
    ...
    sprintf(t_buf,"%s   Ver:%s", /**/g_cur_type->name, g_cur_type->ver);
...
// 		GUI_DispStringAt((const char*)"长 盛 仪 器", 45, 0);
		GUI_SetFont(&GUI_FontHZ_SimSun_12);
		GUI_DispStringAt((const char*)"深圳市西思特科技有限公司", 20, 52);
// 		GUI_DrawBitmap((const GUI_BITMAP*) &bmlogo, 10, 0);
...
// 		GUI_DispStringAt((const char*)"Allwin Instrument", 45+20, 0+8);
		GUI_SetFont(&GUI_FontHZ_SimSun_12);
		GUI_DispStringAt((const char*)"Xisite Technology Co., Ltd.", 20, 52);
// 		GUI_DrawBitmap((const GUI_BITMAP*) &bmallwin, 2, 2);
...
uint8_t custom_type_main_gui(void)
{
    ...
    if(flag == 0)
    {
        if(g_cur_type->num == CS9933X)
        {
            flag = 1;
            xi_si_te_ke_ji_main_ui();//深圳西思特科技定制机
        }
    }
    #endif
