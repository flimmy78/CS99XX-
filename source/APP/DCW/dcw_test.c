...
static uint16_t dcw_zeo_t;///< 第〇阶段的累计时间
static uint16_t dcw_one_t;///< 第一阶段的累计时间
static uint16_t dcw_two_t;///< 第二阶段的累计时间
static uint16_t dcw_thr_t;///< 第三阶段的累计时间
static uint16_t dcw_for_t;///< 第四阶段的累计时间
...
/**
  * @brief  计算DCW在测试过程中电压变化的步进值和开始值与目标值
  * @param  [in] dcw_par 当前步的设置数据
  * @param  [in] test_data 测试数据
  * @retval 无
  */
...
void dcw_test_irq(DCW_STRUCT *dcw_par, TEST_DATA_STRUCT *test_data)
{
	...
	if(0 == dcw_par->testing_time && (!dcw_test_flag.fail_cont))
	{
		...
		/* 使程序进入第一阶段电压上升 */
		else if(test_data->test_time <= dcw_one_t)/* 当上升时间设为0时，就不能进入if,因此要+1 */
		{
...
void dcw_test_details(DCW_STRUCT *dcw_par, TEST_DATA_STRUCT *test_data)
{
	switch(test_data->gradation)
	{
        ...
        {
            dcw_test_flag.fail_cont = 1;//发生失败继续把这个标记置1
            
            /* 间隔时间不为0就从间隔时间开始 */
            if(dcw_par->interval_time > 0)
            {
                test_data->ready_ok = 1;
                test_data->test_time = dcw_thr_t + 1;/* 跳过第1阶段 */
